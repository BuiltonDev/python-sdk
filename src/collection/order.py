...
class Order(Components):
    ...
    def get_deliveries(self, url_params=None):
        return self.simple_query(_id=self._id, resource='deliveries', url_params=url_params, json=True)

    def pay(self, body, url_params=None, json=False):
        return self.simple_query(_type='post', _id=self._id, resource='pay', body=body, url_params=url_params,
                                 json=json)

    def cancel(self, body, url_params=None, json=False):
        return self.simple_query(_type='post', _id=self._id, resource='cancel', body=body, url_params=url_params,
                                 json=json)

    def create_delivery(self, body, url_params=None):
        return self.simple_query(_type='post', _id=self._id, resource='deliveries', body=body, url_params=url_params,
                                 json=True)
